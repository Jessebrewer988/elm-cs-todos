...
var $author$project$Main$LoadTodos = function (a) {
	return {$: 'LoadTodos', a: a};
};
...
var $author$project$Main$init = function (maybeTodos) {
	return _Utils_Tuple2(
		{
			deleteCandidate: $elm$core$Maybe$Nothing,
			text: '',
			title: 'Elm Todos',
			todos: A2(
				...
				_List_fromArray(
					[
						{completed: false, id: 0, title: 'Learn Elm'}
					]),
				maybeTodos)
		},
...
var $author$project$Main$loadTodos = _Platform_incomingPort(
	'loadTodos',
			...
			todos: _Utils_ap(
				...
				_List_fromArray(
					[
						...
						completed: false,
						id: newId,
						title: $elm$core$String$trim(model.text)
					}
					]))
		});
};
...
var $author$project$Main$toggleWith = F3(
	function (id, isChecked, todo) {
		return _Utils_eq(todo.id, id) ? _Utils_update(
			todo,
			{completed: isChecked}) : todo;
	});
			...
			case 'ToggleIsDone':
				...
				var newModel = _Utils_update(
					model,
					{
						todos: A2(
							$elm$core$List$map,
							A2($author$project$Main$toggleWith, id, isChecked),
							model.todos)
					});
				return _Utils_Tuple2(
					newModel,
					$author$project$Main$saveTodos(newModel.todos));
			case 'AddTodo':
				if ($elm$core$String$trim(model.text) !== '') {
					var newModel = $author$project$Main$addTodo(model);
					return _Utils_Tuple2(
						newModel,
						$author$project$Main$saveTodos(newModel.todos));
				} else {
			...
			case 'RequestDelete':
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							deleteCandidate: $elm$core$Maybe$Just(id)
						}),
					$elm$core$Platform$Cmd$none);
			case 'CancelDelete':
				return _Utils_Tuple2(
...
var $author$project$Main$deleteDialogConfig = function (id) {
	return {
		body: $elm$core$Maybe$Just(
			A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Are you sure you want to delete this todo? This action cannot be undone.')
					]))),
		closeMessage: $elm$core$Maybe$Just($author$project$Main$CancelDelete),
		containerClass: $elm$core$Maybe$Just('cs-dialog'),
		footer: $elm$core$Maybe$Just(
			A2(
				$elm$html$Html$menu,
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('footer-btns')
					]),
				_List_fromArray(
					[
						A2(
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$class('cs-btn'),
...
var $author$project$Main$viewDeleteModal = function (model) {
	var _v0 = model.deleteCandidate;
	if (_v0.$ === 'Nothing') {
		...
		return $alex_tan$elm_dialog$Dialog$view(
			$elm$core$Maybe$Just(
				$author$project$Main$deleteDialogConfig(id)));
	}
};
var $author$project$Main$viewProgress = function (model) {
	var total = $elm$core$List$length(model.todos);
	...
	var statusMessage = (!total) ? 'Add todo to see your progress' : ((percentage === 100) ? 'All tasks complete - great job!' : 'Keep up the good work!');
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$class('cs-progress-container')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('cs-progress-text')
					]),
				_List_fromArray(
					[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('cs-progress-bar')
					]),
				_List_fromArray(
					[
						A2(
...
var $author$project$Main$viewFooter = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				$author$project$Main$viewProgress(model),
				$elm$html$Html$text('Made with Elm and CS16.css')
			]));
};
var $author$project$Main$RequestDelete = function (a) {
	return {$: 'RequestDelete', a: a};
};
var $author$project$Main$ToggleIsDone = F2(
	function (a, b) {
		return {$: 'ToggleIsDone', a: a, b: b};
	});
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$class('cs-checkbox')
					]),
				_List_fromArray(
					[
						A2(
						...
						_List_fromArray(
							[
								...
								'checkbox' + $elm$core$String$fromInt(todo.id)),
								$elm$html$Html$Attributes$type_('checkbox'),
								$elm$html$Html$Attributes$checked(todo.completed),
								$elm$html$Html$Events$onCheck(
								$author$project$Main$ToggleIsDone(todo.id)),
								$elm$html$Html$Attributes$class('cs-checkbox')
							]),
						_List_Nil),
						A2(
						...
						_List_fromArray(
							[
								...
								'checkbox' + $elm$core$String$fromInt(todo.id)),
								$elm$html$Html$Attributes$class('cs-checkbox__label')
							]),
						_List_fromArray(
							[
								A2(
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$class('cs-btn delete-btn'),
								$elm$html$Html$Events$onClick(
								$author$project$Main$RequestDelete(todo.id))
							]),
						_List_fromArray(
							[
								$elm$html$Html$text('X')
						...
						_List_fromArray(
							[
								$elm$html$Html$Attributes$class('cs-hr')
							]),
						_List_Nil)
					]))
			]));
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				$author$project$Main$viewDeleteModal(model),
				A2(
				$elm$html$Html$span,
				_List_fromArray(
					[
					...
					[
						...
						_List_fromArray(
							[
								...
								$elm$html$Html$Attributes$class('cs-input'),
...
var $author$project$Main$main = $elm$browser$Browser$element(
	{
		...
		subscriptions: function (_v0) {
			return $author$project$Main$loadTodos($author$project$Main$LoadTodos);
		},
